...
class App:
    def __init__(self):
        ...
        self.npcs = [
            ...
            NPC(8, 5, "Elder", ["To defeat the Demon Lord in the lava region,", "you must traverse the cave to the east.", "I have unsealed the cave for you."], False, 0),
            NPC(25, 5, "Guard", ["The castle is to the north.", "Rest there to recover your HP and MP."], False, 1),
            NPC(14, 11, "Girl", ["I love the white snow in the north!", "Have you seen the Snow Ghost?"], True, 2),
            NPC(15, 25, "Merchant", ["I have no items to sell right now...", "But you can find treasures in the islands!"], False, 3)
        ]
    ...
    def create_graphics(self):
        ...
        # Changed from Color 9 (Beige) to Color 13 (Light Grey) 
        # to distinguish from Brown Treasure Chests (Color 9)
        pyxel.images[0].rect(48, 0, 8, 8, 13) 
        pyxel.images[0].pset(48, 0, 7); pyxel.images[0].pset(55, 7, 7) # White corner dots for texture
        ...
        # Replaced base Color 8 (Red) with Color 2 (Dark Purple/Glow) 
        # to ensure contrast with the Red Player (Color 8)
        pyxel.images[0].rect(80, 0, 8, 8, 2)
        pyxel.images[0].circ(82, 3, 1, 14); pyxel.images[0].circ(85, 6, 1, 10) # Pink/Yellow bubbles
        ...
        pyxel.images[0].rect(114, 4, 4, 4, 11) # Green body
        ...
        pyxel.images[0].rect(2, 12, 4, 4, 8)  # Body (Red - Color 8)
        pyxel.images[0].pset(2, 10, 12); pyxel.images[0].pset(5, 10, 12) # Two blue eyes
        pyxel.images[0].pset(1, 11, 14); pyxel.images[0].pset(6, 11, 14) # Rosy cheeks (Pink)
        
        # 1: Up (Back) - No eyes
        pyxel.images[0].rect(8, 8, 8, 8, 0)
        pyxel.images[0].rect(10, 9, 4, 3, 15) # Head (White)
        pyxel.images[0].rect(10, 12, 4, 4, 8) # Body (Red - Color 8)
        ...
        pyxel.images[0].rect(18, 12, 4, 4, 8) # Body (Red - Color 8)
        pyxel.images[0].pset(18, 10, 12) # Leading blue eye
        pyxel.images[0].pset(17, 11, 14) # Cheek
        
        # 3: Right (Side) - One blue eye (right)
        pyxel.images[0].rect(24, 8, 8, 8, 0)
        pyxel.images[0].rect(26, 9, 4, 3, 15) # Face
        pyxel.images[0].rect(26, 12, 4, 4, 8) # Body (Red - Color 8)
        pyxel.images[0].pset(29, 10, 12) # Leading blue eye
        pyxel.images[0].pset(30, 11, 14) # Cheek

        # NPC Graphics (8x8) - 4 Directions in Bank 0, Y=40
        # Body Color: Green=11, Face Color: White=15 (Head stays white)
        
        # 0: Down (Front) - Two blue eyes
        pyxel.images[0].rect(0, 40, 8, 8, 0)
        pyxel.images[0].rect(2, 41, 4, 3, 15) # Head (White)
        pyxel.images[0].rect(2, 44, 4, 4, 11) # Body (Green - Color 11)
        pyxel.images[0].pset(2, 42, 12); pyxel.images[0].pset(4, 42, 12) # Two blue eyes
        pyxel.images[0].pset(1, 43, 14); pyxel.images[0].pset(6, 43, 14) # Cheeks
        
        # 1: Up (Back) - No eyes
        pyxel.images[0].rect(8, 40, 8, 8, 0)
        pyxel.images[0].rect(10, 41, 4, 3, 15) # Head (White)
        pyxel.images[0].rect(10, 44, 4, 4, 11) # Body (Green - Color 11)
        ...
        pyxel.images[2].circ(132, 38, 4, 0); pyxel.images[2].circ(156, 38, 4, 0)
        # 控えめなハイライト (Subtle sparkles)
        pyxel.images[2].pset(131, 37, 7); pyxel.images[2].pset(155, 37, 7)
        
        # ぽっと染まった大きなほっぺ (Large rosy cheeks)
        pyxel.images[2].circ(115, 48, 6, 14); pyxel.images[2].circ(173, 48, 6, 14)
        
        # 小さな牙と「w」の口 (Tiny fangs and 'w' mouth) - Positioned higher
        pyxel.images[2].line(140, 48, 144, 52, 0); pyxel.images[2].line(144, 52, 148, 48, 0)
        pyxel.images[2].pset(138, 47, 7); pyxel.images[2].pset(150, 47, 7) # Cute fangs

        # ぷにぷにの尻尾の根本 (Chubby tail base)
        pyxel.images[2].circ(178, 80, 12, 8)
        # ハート型の尻尾の先端 (Heart-shaped Tail Tip)
        tail_x, tail_y = 192, 75
        ...
        # Custom Large Font for Title (R, P, G, .) in Image Bank 1
        # Each char is 16x16, starting from Y=32 to avoid NPC portraits
        pyxel.images[1].rect(0, 32, 64, 16, 0) # Clear area
        # R with a hole
        pyxel.images[1].rect(2, 34, 4, 12, 10)
        pyxel.images[1].rect(2, 34, 10, 4, 10)
        pyxel.images[1].rect(10, 34, 4, 8, 10)
        pyxel.images[1].rect(2, 38, 10, 4, 10)
        pyxel.images[1].line(6, 42, 12, 46, 10)
        pyxel.images[1].line(7, 42, 13, 46, 10)
        pyxel.images[1].rect(4, 36, 2, 2, 0) # Black hole in R
        # . (dot)
        pyxel.images[1].rect(18, 44, 2, 2, 10)
        # P with a hole
        pyxel.images[1].rect(34, 34, 4, 12, 10)
        pyxel.images[1].rect(34, 34, 10, 4, 10)
        pyxel.images[1].rect(42, 34, 4, 8, 10)
        pyxel.images[1].rect(34, 38, 10, 4, 10)
        pyxel.images[1].rect(36, 36, 2, 2, 0) # Black hole in P
        # G with holes
        pyxel.images[1].rect(50, 34, 12, 4, 10)
        pyxel.images[1].rect(50, 34, 4, 12, 10)
        pyxel.images[1].rect(50, 42, 12, 4, 10)
        pyxel.images[1].rect(58, 40, 4, 6, 10)
        pyxel.images[1].rect(54, 40, 6, 2, 10)
        pyxel.images[1].rect(54, 40, 2, 2, 0) # Black hole in G
        pyxel.images[1].rect(54, 41, 2, 2, 0) # Black hole in G

        # ============================================================
        #   NPC PORTRAITS (32x32 each) in Image Bank 1
        # ============================================================
        # [0, 0] Village Head / Elder (Face ID: 0)
        pyxel.images[1].rect(0, 0, 32, 32, 0)
        pyxel.images[1].circ(16, 16, 14, 15) # Face
        pyxel.images[1].pset(10, 14, 12); pyxel.images[1].pset(22, 14, 12) # Blue eyes
        pyxel.images[1].rect(6, 18, 20, 10, 13) # White beard
        pyxel.images[1].circ(16, 22, 4, 14) # Mouth area

        # [32, 0] Guard / Warrior (Face ID: 1)
        pyxel.images[1].rect(32, 0, 32, 32, 0)
        pyxel.images[1].circ(48, 16, 14, 13) # Helmet (Grey)
        pyxel.images[1].rect(40, 12, 16, 8, 15) # Face slit
        pyxel.images[1].pset(42, 16, 0); pyxel.images[1].pset(54, 16, 0) # Serious eyes

        # [64, 0] Villager Girl (Face ID: 2)
        pyxel.images[1].rect(64, 0, 32, 32, 0)
        pyxel.images[1].circ(80, 16, 14, 15) # Face
        pyxel.images[1].circ(80, 10, 15, 12) # Blue Hair
        pyxel.images[1].pset(74, 16, 12); pyxel.images[1].pset(86, 16, 12) # Eyes
        pyxel.images[1].circ(72, 20, 3, 14); pyxel.images[1].circ(88, 20, 3, 14) # Cheeks

        # [96, 0] Merchant (Face ID: 3)
        pyxel.images[1].rect(96, 0, 32, 32, 0)
        pyxel.images[1].circ(112, 16, 14, 15) # Face
        pyxel.images[1].rect(100, 4, 24, 6, 4) # Brown Hat
        pyxel.images[1].pset(106, 16, 0); pyxel.images[1].pset(118, 16, 0) # Beady eyes
        pyxel.images[1].line(108, 22, 116, 22, 14) # Smile line
    ...
    def update(self):
        ...
        if self.state == STATE_TITLE:
            ...
            if pyxel.btnp(pyxel.KEY_Z) or pyxel.btnp(pyxel.KEY_RETURN) or pyxel.btnp(pyxel.GAMEPAD1_BUTTON_A) or pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
                pyxel.play(3, 1)
    ...
    def draw_custom_title(self, x, y):
        ...
        # New Y position for title characters (32) to avoid NPC portraits
        if step >= 1: pyxel.blt(x, y, 1, 0, 32, 16, 16, 0)      # R
        if step >= 2: pyxel.blt(x + 12, y, 1, 16, 32, 16, 16, 0) # .
        if step >= 3: pyxel.blt(x + 20, y, 1, 16, 32, 16, 16, 0) # .
        if step >= 4: pyxel.blt(x + 32, y, 1, 32, 32, 16, 16, 0) # P
        if step >= 5: pyxel.blt(x + 44, y, 1, 16, 32, 16, 16, 0) # .
        if step >= 6: pyxel.blt(x + 52, y, 1, 16, 32, 16, 16, 0) # .
        if step >= 7: pyxel.blt(x + 60, y, 1, 16, 32, 16, 16, 0) # .
        if step >= 8: pyxel.blt(x + 72, y, 1, 48, 32, 16, 16, 0) # G
    ...
    def draw(self):
        ...
        if self.state == STATE_TITLE:
            ...
            if pyxel.btnp(pyxel.KEY_RETURN) or pyxel.btnp(pyxel.KEY_Z) or pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
                self.state = STATE_TITLE
                self.title_frame = 0 # Animation counter for title
                # Reset party
                self.party = [
    ...
    def draw_dialog(self):
        # Dialog box at bottom with rounded-ish corners
        pyxel.rect(11, 180, 234, 60, 0)
        pyxel.rect(10, 181, 236, 58, 0)
        pyxel.rectb(11, 180, 234, 60, 7)
        pyxel.rectb(10, 181, 236, 58, 7)
        # Fix corners
        pyxel.pset(10, 180, 0); pyxel.pset(245, 180, 0)
        pyxel.pset(10, 239, 0); pyxel.pset(245, 239, 0)
        ...
        if self.active_npc:
            # Face box at top right with rounded effect
            pyxel.rect(211, 10, 34, 36, 0)
            pyxel.rect(210, 11, 36, 34, 0)
            pyxel.rectb(211, 10, 34, 36, 7)
            pyxel.rectb(210, 11, 36, 34, 7)
            
            # Draw portrait from Bank 1 (each 32x32)
            # index is stored in active_npc.face_id
            u = (self.active_npc.face_id % 8) * 32
            v = (self.active_npc.face_id // 8) * 32
            pyxel.blt(212, 12, 1, u, v, 32, 32, 0)
            # Display Name
            pyxel.text(210, 48, self.active_npc.name, 10)

    def draw_party_status(self):
        # Expanded to 3 lines (y=224, height=32) with rounded effect
        pyxel.rect(1, 224, pyxel.width-2, 32, 0)
        pyxel.rect(0, 225, pyxel.width, 30, 0)
        pyxel.rectb(1, 224, pyxel.width-2, 32, 7)
        pyxel.rectb(0, 225, pyxel.width, 30, 7)
    ...
    def draw_battle(self):
        ...
        # Status window with rounded-ish corners
        pyxel.rect(1, 200, pyxel.width-2, 56, 0)
        pyxel.rect(0, 201, pyxel.width, 54, 0)
        pyxel.rectb(1, 200, pyxel.width-2, 56, 7)
        pyxel.rectb(0, 201, pyxel.width, 54, 7)

        for i, char in enumerate(self.party):
            x = 8 + i * 80
            c = 7 if char.hp > 0 else 8
            if self.battle_phase == 1 and self.menu_mode == "main" and i == self.active_character_idx:
                # Heart-shaped battle cursor
                hx, hy = x-8, 208
                pyxel.pset(hx, hy, 14); pyxel.pset(hx+2, hy, 14)
                pyxel.pset(hx-1, hy+1, 14); pyxel.pset(hx, hy+1, 14); pyxel.pset(hx+1, hy+1, 14); pyxel.pset(hx+2, hy+1, 14); pyxel.pset(hx+3, hy+1, 14)
                pyxel.pset(hx, hy+2, 14); pyxel.pset(hx+1, hy+2, 14); pyxel.pset(hx+2, hy+2, 14)
                pyxel.pset(hx+1, hy+3, 14)
            pyxel.text(x, 208, char.name, c)
            pyxel.text(x, 218, f"HP:{char.hp}/{char.max_hp}", c)
            pyxel.text(x, 228, f"MP:{char.mp}/{char.max_mp}", 12)

        # Message window with rounded-ish corners
        pyxel.rect(1, 0, pyxel.width-2, 50, 0)
        pyxel.rect(0, 1, pyxel.width, 48, 0)
        pyxel.rectb(1, 0, pyxel.width-2, 50, 7)
        pyxel.rectb(0, 1, pyxel.width, 48, 7)
        for i, msg in enumerate(self.battle_messages):
            pyxel.text(4, 4 + i * 10, msg, 7)

        if self.battle_phase == 1:
            # Menu window with rounded-ish corners
            pyxel.rect(9, 130, 68, 65, 0)
            pyxel.rect(8, 131, 70, 63, 0)
            pyxel.rectb(9, 130, 68, 65, 7)
            pyxel.rectb(8, 131, 70, 63, 7)
            
            if self.menu_mode == "main":
                cmds = ["Attack", "Defend", "Magic", "Item", "Flee"]
                for i, c in enumerate(cmds): pyxel.text(20, 135 + i*11, c, 7)
                # Heart-shaped cursor
                hx, hy = 12, 135 + self.battle_cursor*11
                pyxel.pset(hx, hy, 14); pyxel.pset(hx+2, hy, 14)
                pyxel.pset(hx-1, hy+1, 14); pyxel.pset(hx, hy+1, 14); pyxel.pset(hx+1, hy+1, 14); pyxel.pset(hx+2, hy+1, 14); pyxel.pset(hx+3, hy+1, 14)
                pyxel.pset(hx, hy+2, 14); pyxel.pset(hx+1, hy+2, 14); pyxel.pset(hx+2, hy+2, 14)
                pyxel.pset(hx+1, hy+3, 14)
                ...
                if char.spells:
                    hx, hy = 12, 135 + self.sub_cursor*11
                    pyxel.pset(hx, hy, 14); pyxel.pset(hx+2, hy, 14)
                    pyxel.pset(hx-1, hy+1, 14); pyxel.pset(hx, hy+1, 14); pyxel.pset(hx+1, hy+1, 14); pyxel.pset(hx+2, hy+1, 14); pyxel.pset(hx+3, hy+1, 14)
                    pyxel.pset(hx, hy+2, 14); pyxel.pset(hx+1, hy+2, 14); pyxel.pset(hx+2, hy+2, 14)
                    pyxel.pset(hx+1, hy+3, 14)
                ...
                hx, hy = 12, 135 + self.target_cursor*11
                pyxel.pset(hx, hy, 14); pyxel.pset(hx+2, hy, 14)
                pyxel.pset(hx-1, hy+1, 14); pyxel.pset(hx, hy+1, 14); pyxel.pset(hx+1, hy+1, 14); pyxel.pset(hx+2, hy+1, 14); pyxel.pset(hx+3, hy+1, 14)
                pyxel.pset(hx, hy+2, 14); pyxel.pset(hx+1, hy+2, 14); pyxel.pset(hx+2, hy+2, 14)
                pyxel.pset(hx+1, hy+3, 14)
